...
## Corrección 2026-09-02 — SKU duplicados en Neon / Render 503

Se corrigió el sincronizador del catálogo para conservar los SKU históricos de los productos ya existentes y asignar el prefijo `PAR-D26-` únicamente a perfumes nuevos. La versión anterior podía reutilizar un SKU antiguo para otro perfume (por ejemplo `PAR-H-005`) y PostgreSQL rechazaba el arranque por la restricción `uq_productos_sku`.

El `DataInitializer` ahora:
- busca primero por SKU estable;
- evita reutilizar una fila cuyo SKU pertenece a otro perfume del catálogo;
- puede reparar las filas alcanzadas por el despliegue fallido anterior;
- captura fallos individuales de sincronización para que un producto no tumbe toda la API;
- respeta `APP_SEED_DATA=false` como interruptor de emergencia.
