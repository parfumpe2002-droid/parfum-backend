...
public class Producto {
    ...
    public void recalcularResumenComercial() {
        ...
        if (presentaciones.isEmpty()) {
            List<ProductoDecant> decantsDisponibles = decants.stream()
                    .filter(ProductoDecant::isActivo)
                    .filter(item -> item.getPrecio() != null && item.getPrecio().signum() > 0)
                    .toList();
            this.stock = decantsDisponibles.stream()
                    .map(ProductoDecant::getStock)
                    .filter(value -> value != null && value > 0)
                    .mapToInt(Integer::intValue)
                    .sum();
            this.precio = decantsDisponibles.stream()
                    .map(ProductoDecant::getPrecio)
